...
logger = get_logger("abc_preprocess")
...
def preprocess_case(case_rec, cfg):
    ...
    pad_vox = compute_padding_vox(cfg["abc_preprocess"]["roi_padding_mm"], spacing)
    ...
    for tooth_id in tooth_labels:
        ...
        if bool(cfg["abc_preprocess"].get("resample_to_target", False)):
            A_roi, T_roi, B_roi, spacing, resampled, scale = _resample_roi(
                A_roi,
                T_roi,
                B_roi,
                spacing,
                cfg["abc_preprocess"]["target_spacing_mm"],
            )
        else:
            resampled = False
        ...
        save_volume(os.path.join(tooth_dir, f"A_t.{fmt}"), A_roi, spacing=spacing)
        save_volume(os.path.join(tooth_dir, f"T_t.{fmt}"), T_roi.astype(np.uint8), spacing=spacing)
        save_volume(os.path.join(tooth_dir, f"B_t.{fmt}"), B_roi.astype(np.uint8), spacing=spacing)
        ...
        roi_meta = {
            ...
            "padding_mm": float(cfg["abc_preprocess"]["roi_padding_mm"]),
            "target_spacing_mm": [float(v) for v in cfg["abc_preprocess"]["target_spacing_mm"]],
        ...
        logger.info("abc preprocess case=%s tooth=%s", case_id, tooth_id)
