...
def _shape_prior_constraint_loss(logits, batch, device, cfg):
    ...
    prob = torch.sigmoid(logits)
    return float(weight) * torch.mean((prob ** 2) * (1.0 - shape_prior))
...
def train_one_epoch(model, loader, optimizer, device, loss_fn, cfg, global_step):
    ...
    for batch in loader:
        ...
        loss = loss + _shape_prior_constraint_loss(logits, batch, device, cfg)
        loss.backward()
        optimizer.step()
